...
const browser_shortcuts = async (bundle_id: string) => {
  await $`defaults write ${bundle_id} NSUserKeyEquivalents -dict-add "Duplicate Tab" '@$d'`.quiet().nothrow();
  await $`defaults write ${bundle_id} NSUserKeyEquivalents -dict-add "New Tab to the Right" '@t'`.quiet().nothrow();
  await $`defaults write ${bundle_id} NSUserKeyEquivalents -dict-add "Bookmark All Tabs..." '@$b'`.quiet().nothrow();
};
...
await $`/System/Library/PrivateFrameworks/SystemAdministration.framework/Resources/activateSettings -u`.quiet().nothrow();
